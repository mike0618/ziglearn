...
//
// Payload Captures - uses |value| to capture value from smth.
//
test "optional-if" {
...
// with if statement and err union. Err capture is required here
test "error union if" {
...
// while loops and err unions. Else with err capture is required here
...
test "switch capture" {
    ...
    const x = switch (b) {
        ...
        .c => 2,
        // if these are of the same type, they may be inside the same capture group.
        .a, .d => |num| blk: {
...
// captured with |val| are IMMUTABLE like fn args, use ptr to modify them.
